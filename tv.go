...
func main() {
	...
	RootCmd.Flags().IntVar(&args.SkipNum, "in", 0, "(optional) Ignore first N Lines")
	RootCmd.Flags().IntVar(&args.NLine, "nl", 0, "(optional) Only display first N lines")
	RootCmd.Flags().IntSliceVar(&args.ShowNum, "dc", []int{}, "(optional) Only display specific columns (multiple arguments support, separated by comma)")
	RootCmd.Flags().IntSliceVar(&args.HideNum, "hc", []int{}, "(optional) Do not display specific columns (multiple arguments support, separated by comma)")
